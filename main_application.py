...
class MainApplication(tk.Tk):
    ...
    def createWidget(self):
        ...
        self.action = ttk.Button(self.monty, text="后退一次",command=self.backward_motion)
        # Modified Button Click Function # 1
        # Position Button in second row, second column (zero-based)
        self.action.grid(column=1, row=4)
        self.action = ttk.Button(self.monty, text="前进一次",command=self.forward_motion)
        # Modified Button Click Function # 1
        # Position Button in second row, second column (zero-based)
        ...
        ttk.Label(self.monty, text="运动速度\n(mm/min):").grid(column=2, row=0) # 1
