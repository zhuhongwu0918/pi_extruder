...
class OOP():
    ...
    def createWidget(self):
        ...
        self.tab4 = ttk.Frame(tabControl) # Add a third tab
        tabControl.add(self.tab4, text='Tab 4') # Make second tab visible

        #--------------------------------------------------------------
        self.fig = Figure(figsize=(10, 5), facecolor='white')
        #--------------------------------------------------------------
        self.axis = self.fig.add_subplot(111) # 1 row, 1 column
        self.xValues = [1,2,3,4]
        self.yValues0 = [6,7.5,8,7.5]
        self.yValues1 = [5.5,6.5,2,9.6]
        self.yValues2 = [6.5,7,8,7]
        self.t0, = self.axis.plot(self.xValues, self.yValues0,color='purple')
        self.t1, = self.axis.plot(self.xValues, self.yValues1,color='red')
        self.t2, = self.axis.plot(self.xValues, self.yValues2,color='blue')   #变量加逗号可以把长度为1的元组中的元素提取出来https://www.cnblogs.com/azureology/p/12343730.html
        #dynamical set scale------------------------------------------------
        self.minx=min(self.xValues)
        self.maxx=max(self.xValues)
        self.axis.set_xlim(self.minx,self.maxx)
        self.y_all=self.yValues0+self.yValues1+self.yValues2
        print(self.y_all)
        self.miny=min(self.y_all)
        self.maxy=max(self.y_all)

        self.axis.set_ylim(self.miny,self.maxy) #限制显示范围
        #dynamical set scale-------------------------------------------------
        self.axis.set_ylabel('Vertical Label')
        self.axis.set_xlabel('Horizontal Label')
        self.axis.grid()
        
        self.fig.legend((self.t0, self.t1, self.t2), ('First line', 'Second line', 'Third line'), 'upper right')
